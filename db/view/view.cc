...
void view_updates::generate_update(
    ...
    if (has_new_row) {
        ...
        // This is the Alternator-only support for *multiple* regular base
        // columns that become view key columns (e.g. a composite GSI key with
        // several HASH/RANGE attributes extracted from the item's regular
        // attributes). The timestamp we use is the *maximum* across all of
        // them, as explained in pull-request #17172 (originally for 2 columns,
        // generalized here to an arbitrary number for composite GSI keys).

        // Alternator isn't supposed to have more than 8 regular base columns in a view key.
    ...
    if (has_old_row) {
        ...
        // This is the Alternator-only support for multiple regular base
        // columns that become view key columns (composite GSI keys). See
        // explanation in view_updates::compute_row_marker().
