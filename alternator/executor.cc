...
// Below is specified legacy behavior of this tag. From now on this logic
// is superseded by the NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY
// tag below.
...
// This will be set to a string value indicating the number of geniune
// range keys specified by the user during GSI creation. This indicates the
// prefix of columns that are specified as clustering keys to be reported as
// range keys for DescribeTable. The remaining columns that are not reported
// as range keys are the needed base table's keys.
extern const sstring NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY("system:number_of_non_spurious_user_specified_range_keys_tag_key");
...
// by up to 4 RANGE attributes forming the sort key (8 KeySchema entries in
// total). All HASH entries must appear before all RANGE entries (i.e., the
...
static std::pair<std::vector<std::string>, std::vector<std::string>> parse_gsi_composite_key_schema(const rjson::value& obj, std::string_view supplementary_context) {
    ...
    for (size_t i = 0; i < key_schema->Size(); ++i) {
        ...
        if (key_type == "HASH") {
            if (seen_range_key) {
                throw api_error::validation("All HASH keys in KeySchema must come before all RANGE keys");
            }
            if (hash_keys.size() >= 4) {
...
future<executor::request_return_type> executor::create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request, bool enforce_authorization, bool warn_authorization,
    ...
    if (gsi) {
        ...
        for (const rjson::value& g : gsi->GetArray()) {
            ...
            // key columns, appended below as spurious clustering columns for
            // materialized-view correctness). DescribeTable relies on this tag via
            // genuine_range_key_count() (executor_util.cc) to know where to stop
            ...
            uint32_t non_spurious_user_specified_range_key_count = view_range_keys.size();
            std::map<sstring, sstring> tags;
            tags[NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY] = std::to_string(non_spurious_user_specified_range_key_count);
...
future<executor::request_return_type> rmw_operation::execute(service::storage_proxy& proxy,
        ...
        if (_write_isolation == write_isolation::UNSAFE_RMW) {
            ...
            return get_previous_item(proxy, client_state, schema(), _pk, _ck, permit, db::consistency_level::LOCAL_QUORUM, _consumed_capacity._total_bytes).then(
                ...
                if (!m) {
                    ...
                    per_table_stats.conditional_check_failed++;
    ...
    return proxy.cas(schema(), std::move(*cas_shard), *this, read_command, to_partition_ranges(*schema(), _pk),
            ...
            db::consistency_level::LOCAL_SERIAL, db::consistency_level::LOCAL_QUORUM, timeout, timeout, true, std::move(cdc_opts)).then([this, read_command, &wcu_total, &global_stats, &per_table_stats] (service::storage_proxy::cas_result cas_result) mutable {
