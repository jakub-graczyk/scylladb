...
extern const sstring NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY;

uint32_t genuine_range_key_count(const schema& schema, const std::map<sstring, sstring>* tags) {
    // When GSI doesn't carry the NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY
    // tag, to preserve behavior of existing tables we translate the
    // SPURIOUS_RANGE_KEY_ADDED_TO_GSI_AND_USER_DIDNT_SPECIFY_RANGE_KEY_TAG_KEY
    // to "0" in NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY
    ...
    // the user-specified one, purely for materialized-view correctness.
    ...
    bool ignore_range_keys_as_spurious = tags != nullptr && tags->contains(SPURIOUS_RANGE_KEY_ADDED_TO_GSI_AND_USER_DIDNT_SPECIFY_RANGE_KEY_TAG_KEY);
    uint32_t range_keys_to_report = ignore_range_keys_as_spurious ? 0 : 1;
    if (tags != nullptr) {
        auto it = tags->find(NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY);
        if (it != tags->end() && !it->second.empty()) {
            try {
                ...
                on_internal_error(elogger, fmt::format(
                        ...
                        NUMBER_OF_NON_SPURIOUS_USER_SPECIFIED_RANGE_KEYS_TAG_KEY, it->second, schema.ks_name(), schema.cf_name()));
            }
        }
    }
...
void describe_key_schema(rjson::value& parent, const schema& schema, std::unordered_map<std::string, std::string>* attribute_types, const std::map<sstring, sstring>* tags) {
    ...
    uint32_t range_keys_to_report = genuine_range_key_count(schema, tags);
    size_t i = 0;
